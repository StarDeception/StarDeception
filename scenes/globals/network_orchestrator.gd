...
signal set_gameserver_number_players_iniverse(nb_players)
signal set_gameserver_serverzone(serverzone)

const UUID_UTIL = preload("res://addons/uuid/uuid.gd")
const MQTT = preload("res://addons/mqtt/mqtt.tscn")

var sdo_server_url: String = ""
var server_name: String = ""
var server_ip: String = ""
var server_port: int = 7050
var is_sdo_active: bool = false

var server_mqtt_url: String = ""
var server_mqtt_port
var server_mqtt_username: String = ""
var server_mqtt_password: String = ""
var server_mqtt_verbose_level: int = 2

var server_sdo_url: String = ""
var server_sdo_port: int = 7050
var server_sdo_username: String = ""
var server_sdo_password: String = ""
var server_sdo_verbose_level: int = 2

var metrics_enabled: bool = false
var metrics_url: String = ""
var metrics_port: int = 7050
var metrics_username: String = ""
var metrics_password: String = ""
var metrics_verbose_level: int = 2
...
func _sdo_register():
	...
	mqtt_client_sdo.subscribe("sdo/playerslist")
	mqtt_client_sdo.subscribe("sdo/propslist")
...
func _on_mqtt_sdo_received_message(topic, message):
	if topic == "sdo/serverslist":
		...
		#}]
		var srvs_list = JSON.parse_string(message)
		for server in srvs_list:
		...
		if is_sdo_active:
			...
			mqtt_client_sdo.unsubscribe("sdo/playerslist")
...
func _create_local_player_not_exists_in_universe(uuid, player_name, spawn_point, id):
	...
	if spawn_point >= 0 and spawn_point < GameOrchestrator.SPAWN_POINTS_LIST.size():
		...
		if spawn_point_node_path == "":
			var parent_entity_spawn_position: Vector3 = universe_scene.get_node("PlanetB").global_position
			if randf() < 0.5:
				parent_entity_spawn_position = universe_scene.get_node("PlanetA").global_position
...
func _search_another_server_for_coordinates(x, y, z):
	for s in servers_list.values():
		if s.id == server_sdo_id:
			continue
		if float(s.x_start) <= x and \
			x < float(s.x_end) and \
			float(s.y_start) <= y and \
			y < float(s.y_end) and \
			float(s.z_start) <= z and \
			z < float(s.z_end):
			return s
	return null
...
func get_gameserver_number_players_universe(nb_players):
	set_gameserver_number_players_iniverse.emit(nb_players)

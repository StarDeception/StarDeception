...
const GAME_STATES_SCENES_PATHS: Dictionary = {
	...
	GameStates.TROLL : "res://ui/trolling_page/trolling_page.tscn",
}
...
func _ready():
	if OS.has_feature("editor"):
		var setting = ProjectSettings.get_setting("application/run/main_scene")
		var ressource_name = ResourceUID.id_to_text(ResourceLoader.get_resource_uid(get_tree().current_scene.scene_file_path))
		if ressource_name != setting and not OS.has_feature("dedicated_server"):
			return
